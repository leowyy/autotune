    ...
    def run(self, N, block_until_done=True):
        ...
        while n_queued < N:
            ...
            while qlen < self.max_queue_len and n_queued < N:
                ...
                time_exceeded =  self.cum_time > self.max_time
                if time_exceeded:
                    print("Exiting run, exceeded time budget.")
                    stopped = True
                    break
            ...
            if self.async:
                ...
                if self.verbose:
                    print("num iteration: {}, time elapsed: {:.2f}s, f_current: {:.5f}, f_best: {:.5f}".format(
                        self.num_iterations, self.cum_time, self.trials.losses()[-1], min(self.trials.losses())))
